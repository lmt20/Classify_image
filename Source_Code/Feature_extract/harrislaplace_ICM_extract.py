...
def cal_harrislaplace_ICM_totalimage(img):
  ...
  for i in range(len(kp)):
    ...
    rs_moments.append(cal_invariant_colormoments(sub_img))
  return np.array(rs_moments)


#HarrisLaplace-ICM extract
def harrislaplace_ICM_extract(path_img, path_output):
  print("begin!")
  ...
  for filename in list_dir:
    ...
    feture_extract = cal_harrislaplace_ICM_totalimage(img)
    np.savez(outfile, feture_extract)
    count +=1
    print(f"extract feature file {count} done!!")

class harrislaplace_ICM_extract:
  def extract(self, img):
    return cal_harrislaplace_ICM_totalimage(img)
  def descriptorSize(self):
      return 24
# path_img = "/media/lmtruong/01D557A8C9369F20/Work/JPEGImages"
# path_output = "/home/lmtruong/Documents/Work_Project/Data/Feature_extract/harrislaplace_ICM_extract"
# harrislaplace_ICM_extract(path_img, path_output)